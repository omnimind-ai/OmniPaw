...
export async function findInstalledRuntime(installDir: string): Promise<string | undefined> {
  const candidates =
    platform === 'win32'
      ? [
          join(installDir, 'omniinfer.cmd'),
          join(installDir, 'omniinfer.bat'),
          join(installDir, 'omniinfer.exe'),
          join(installDir, 'omniinfer.ps1'),
          join(installDir, 'omniinfer.py'),
        ]
