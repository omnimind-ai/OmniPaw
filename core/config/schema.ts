...
function migrateConfig(raw: unknown): unknown {
  ...
  if (version > CURRENT_SETTINGS_VERSION) {
    throw new ConfigValidationError(configError(
      'unsupported_version',
...
function normalizeProviderSource(item: unknown): DesktopProviderSource {
  ...
  if (!isPlainObject(item)) {
    throwValidationError([{ path: 'providers.sources', message: 'Provider source must be an object.', code: 'invalid_type' }])
  }
  const raw = item
  ...
  return {
    ...
    enabled: raw.enabled === undefined ? merged.enabled : raw.enabled as DesktopProviderSource['enabled'],
    ...
    defaultModelId: typeof raw.defaultModelId === 'string' ? raw.defaultModelId : merged.defaultModelId,
...
function normalizeProviderModel(item: unknown): DesktopProviderModel {
  ...
  return {
    ...
    id: typeof raw.id === 'string' ? raw.id : merged.id,
    name: typeof raw.name === 'string' ? raw.name : merged.name,
    providerSourceId: typeof raw.providerSourceId === 'string'
    ...
    enabled: raw.enabled === undefined ? merged.enabled : raw.enabled as DesktopProviderModel['enabled'],
    input: isModelInput(raw.input) ? raw.input : merged.input,
    supportsStreaming: typeof raw.supportsStreaming === 'boolean' ? raw.supportsStreaming : merged.supportsStreaming,
    supportsTools: typeof raw.supportsTools === 'boolean' ? raw.supportsTools : merged.supportsTools,
    supportsReasoning: typeof raw.supportsReasoning === 'boolean' ? raw.supportsReasoning : merged.supportsReasoning,
    contextWindow: typeof raw.contextWindow === 'number' ? raw.contextWindow : merged.contextWindow,
    maxOutputTokens: typeof raw.maxOutputTokens === 'number' ? raw.maxOutputTokens : merged.maxOutputTokens,
...
function validateApp(config: DesktopSettingsConfig, issues: SettingsValidationIssue[]): void {
  if (!['system', 'light', 'dark'].includes(config.app.theme)) {
    issues.push({ path: 'app.theme', message: 'Theme must be system, light, or dark.', code: 'invalid_enum' })
  }
  if (!['system', 'zh-CN', 'en-US'].includes(config.app.language)) {
    issues.push({ path: 'app.language', message: 'Language must be system, zh-CN, or en-US.', code: 'invalid_enum' })
  }
  if (typeof config.app.minimizeToTrayOnStartup !== 'boolean') {
    issues.push({ path: 'app.minimizeToTrayOnStartup', message: 'Value must be boolean.', code: 'invalid_type' })
  }
  if (!isFiniteNumber(config.app.zoom.factor) || config.app.zoom.factor < config.app.zoom.min || config.app.zoom.factor > config.app.zoom.max) {
    issues.push({ path: 'app.zoom.factor', message: 'Zoom factor must be within configured min and max.', code: 'out_of_range' })
  }
  if (!Number.isInteger(config.app.maxRecentMessages) || config.app.maxRecentMessages < 1) {
    issues.push({ path: 'app.maxRecentMessages', message: 'Max recent messages must be a positive integer.', code: 'invalid_number' })
  }
}
...
function validateProviders(config: DesktopSettingsConfig, issues: SettingsValidationIssue[]): void {
  ...
  for (const [index, source] of config.providers.sources.entries()) {
    ...
    if (!source.id) {
      issues.push({ path: `${basePath}.id`, message: 'Provider source ID is required.', code: 'required' })
    }
    if (sourceIds.has(source.id)) {
      issues.push({ path: `${basePath}.id`, message: 'Provider source ID must be unique.', code: 'duplicate' })
    }
    sourceIds.add(source.id)
    if (!source.name) {
      issues.push({ path: `${basePath}.name`, message: 'Provider source name is required.', code: 'required' })
    }
    if (!source.baseUrl) {
      issues.push({ path: `${basePath}.baseUrl`, message: 'Provider source base URL is required.', code: 'required' })
    }
    if (typeof source.enabled !== 'boolean') {
      issues.push({ path: `${basePath}.enabled`, message: 'Provider source enabled state must be boolean.', code: 'invalid_type' })
    }
    if (!['openai-compatible', 'ollama', 'omniinfer'].includes(source.type)) {
      issues.push({ path: `${basePath}.type`, message: 'Provider source type is invalid.', code: 'invalid_enum' })
    }
    if (!['openai-chat-completions', 'openai-responses', 'ollama', 'omniinfer'].includes(source.api)) {
      issues.push({ path: `${basePath}.api`, message: 'Provider source API is invalid.', code: 'invalid_enum' })
    }
    if (!isPlainObject(source.headers)) {
      issues.push({ path: `${basePath}.headers`, message: 'Provider source headers must be an object.', code: 'invalid_type' })
    }
    if (!isPlainObject(source.extraBody)) {
      issues.push({ path: `${basePath}.extraBody`, message: 'Provider source extra body must be an object.', code: 'invalid_type' })
    }
    if (!isPlainObject(source.capabilities)) {
      issues.push({ path: `${basePath}.capabilities`, message: 'Provider source capabilities must be an object.', code: 'invalid_type' })
    }
  }
  ...
  for (const [index, model] of config.providers.models.entries()) {
    ...
    if (!model.id) {
      issues.push({ path: `${basePath}.id`, message: 'Provider model ID is required.', code: 'required' })
    }
    if (!model.name) {
      issues.push({ path: `${basePath}.name`, message: 'Provider model name is required.', code: 'required' })
    }
    if (modelIds.has(model.id)) {
      issues.push({ path: `${basePath}.id`, message: 'Provider model ID must be unique.', code: 'duplicate' })
    }
    ...
    if (typeof model.enabled !== 'boolean') {
      issues.push({ path: `${basePath}.enabled`, message: 'Provider model enabled state must be boolean.', code: 'invalid_type' })
    }
    if (!Array.isArray(model.input)) {
      issues.push({ path: `${basePath}.input`, message: 'Provider model input must be an array.', code: 'invalid_type' })
    }
    if (!isPlainObject(model.capabilities)) {
      issues.push({ path: `${basePath}.capabilities`, message: 'Provider model capabilities must be an object.', code: 'invalid_type' })
    }
  }
...
function validateTools(config: DesktopSettingsConfig, issues: SettingsValidationIssue[]): void {
  if (!isPlainObject(config.tools.enabledByName)) {
    issues.push({ path: 'tools.enabledByName', message: 'Tool enablement must be an object.', code: 'invalid_type' })
  }
}

function validateScheduledTasks(config: DesktopSettingsConfig, issues: SettingsValidationIssue[]): void {
  if (typeof config.scheduledTasks.enabled !== 'boolean') {
    issues.push({ path: 'scheduledTasks.enabled', message: 'Scheduled task enabled state must be boolean.', code: 'invalid_type' })
  }
  if (!Array.isArray(config.scheduledTasks.tasks)) {
    issues.push({ path: 'scheduledTasks.tasks', message: 'Scheduled tasks must be an array.', code: 'invalid_type' })
  }
}
...
function throwValidationError(issues: SettingsValidationIssue[]): never {
  throw new ConfigValidationError(configError(
    'invalid_config',
    'Desktop settings config is invalid.',
    {
      recoverable: false,
      issues,
    },
...
function isModelInput(value: unknown): value is DesktopProviderModel['input'] {
  return Array.isArray(value) && value.every((item) => item === 'text' || item === 'image' || item === 'audio' || item === 'file')
}
