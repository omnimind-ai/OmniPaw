...
function normalizeRegistryShape(raw: unknown, issues: McpValidationIssue[]): McpServerRegistry {
  ...
  const servers =
    raw.servers === undefined || raw.servers === null
      ? []
      : Array.isArray(raw.servers)
        ? raw.servers.map((server, index) =>
            normalizeServerRecord(server, `servers.${index}`, issues)
          )
        : issueAndReturn<McpServerRecord[]>(
            issues,
            'servers',
  ...
  return {
    version:
      typeof raw.version === 'number'
        ? (raw.version as McpRegistryVersion)
        : CURRENT_MCP_REGISTRY_VERSION,
    servers,
  }
}
...
function validateServerRecord(
  ...
  if (typeof server.enabled !== 'boolean') {
    issues.push({
      path: `${path}.enabled`,
      message: 'Server enabled state must be boolean.',
      code: 'invalid_type',
    })
  }
...
function validateStdioTransport(
  ...
  if (!Array.isArray(transport.args) || !transport.args.every((arg) => typeof arg === 'string')) {
    issues.push({
      path: `${path}.args`,
      message: 'Command arguments must be strings.',
      code: 'invalid_type',
    })
  }
...
function validateTimeout(value: number, path: string, issues: McpValidationIssue[]): void {
  if (!Number.isInteger(value) || value < 1_000 || value > 300_000) {
    issues.push({
      path,
      message: 'Timeout must be an integer between 1000 and 300000 milliseconds.',
      code: 'out_of_range',
    })
  }
}
