    ...
    for (const record of records) {
      for (const part of record.content?.message || []) {
        if (
          ...
          !part.embedded_url &&
          (partAttachmentId(part) || part.filename)
        ) {
...
function toolCallsFromEvent(event: BridgeStreamEvent): ToolCall[] {
  const eventRecord = event as Record<string, unknown>
  const direct =
    eventRecord.toolCall ?? eventRecord.tool_call ?? eventRecord.toolCalls ?? eventRecord.tool_calls
...
function normalizeToolCalls(value: unknown): ToolCall[] {
  if (Array.isArray(value)) {
    return value.map((item) => normalizeToolCall(item)).filter((item) => hasToolCallIdentity(item))
  }
...
function normalizeRecordError(error: unknown) {
  const message =
    errorMessage(error) || String((error as Error)?.message || error || 'Request failed.')
