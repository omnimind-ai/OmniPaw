<template>
  ...
  >
    ...
    <ProjectDialog
      ...
      @save="saveProject"
    />
    <v-dialog v-model="settingsDialogOpen" max-width="560">
      <v-card class="settings-dialog-card">
        <v-card-title class="settings-dialog-title">
          <span>{{ t("core.common.settings") }}</span>
          <v-btn icon="mdi-close" size="small" variant="text" @click="settingsDialogOpen = false" />
        </v-card-title>
        <v-card-text class="settings-dialog-content">
          <v-list density="comfortable" class="settings-dialog-list">
            <v-list-subheader>基础设置</v-list-subheader>
            <v-list-item rounded="lg">
              ...
              <template #append>
                <v-btn-toggle v-model="transportMode" density="compact" mandatory variant="outlined">
                  <v-btn
            ...
            <v-list-item rounded="lg">
              ...
              <v-list-item-title>更多基础设置</v-list-item-title>
              <v-list-item-subtitle>后续接入 Provider、上下文和快捷键配置。</v-list-item-subtitle>
            </v-list-item>
          </v-list>
        </v-card-text>
      </v-card>
...
function toggleTheme() {
  const nextTheme = isDark.value ? "PurpleTheme" : "PurpleThemeDark";
