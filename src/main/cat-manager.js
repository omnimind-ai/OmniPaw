...
const catWindowSize = { width: 148, height: 132 }
const catPanelSize = { width: 280, height: 280 }
...
function hideCatWindow() {
  ...
  catVisible = false
  catState = 'hidden'
  broadcastStatus({ source: 'hide' })

  return { ok: true, state: catState, visible: catVisible }
}
  ...
  return new Promise((resolve) => {
    catSnapTimer = setInterval(() => {
      ...
      if (progress >= 1) {
        ...
        broadcastStatus({ source: 'drag-end' })
...
function createCatPanelWindow(placement) {
  ...
  catPanelWindow.on('closed', () => {
    catPanelWindow = null
    broadcastStatus({ panelVisible: false, source: 'cat-panel' })
  })
...
function toggleCatPanelWindow() {
  if (!catWindow || catWindow.isDestroyed() || !catWindow.isVisible()) {
    return { ok: false, error: 'Cat window is not visible' }
  }
  ...
  broadcastStatus({ panelVisible: true, panelSide: placement.side, source: 'cat-panel' })

  return { ok: true, visible: true, side: placement.side, bounds: placement.bounds }
}

/**
 * Register all cat-related IPC handlers.
 * @param {(payload: object) => void} statusChangedCb  called whenever cat status changes
 */
function registerIpcHandlers(statusChangedCb) {
  onStatusChanged = statusChangedCb
  ...
  ipcMain.handle('cat:set-state', (_event, state) => {
    if (!allowedPanelStates.has(state)) {
      return { ok: false, error: 'Invalid cat state', state: catState, visible: catVisible }
    }
  ...
  ipcMain.handle('cat:drag-move', (_event, payload = {}) => {
    ...
    catWindow.setBounds(nextBounds)
    broadcastStatus({ source: 'drag-move' })
  ...
  ipcMain.handle('cat:drag-end', async () => {
    ...
    return targetBounds ? animateCatBounds(targetBounds) : null
  })
  ...
  ipcMain.on('cat:renderer-state', (_event, state) => {
    ...
    broadcastStatus({ source: 'cat-window' })
  })
}
...
module.exports = { registerIpcHandlers, showCatWindow, hideCatWindow, cleanup }
