...
- MUST：`electron/main.ts` 的 handler 调用 core service/manager，不把业务规则写进 preload。
...
- SHOULD：兼容旧调用签名的归一化逻辑集中在 main 或 preload 的现有 normalize 函数附近。
...
4. 在 `electron/main.ts` 注册 handler 并调用 core。
...
- [ ] IPC 新能力完成 constants、shared types、main、preload、renderer bridge 同步。
